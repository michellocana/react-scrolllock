...
function App() {
  ...
  useEffect(() => {
    window.addEventListener('scroll', () => {
      ...
      (scrollAreaRef.current?.clientHeight ?? 0 - document.documentElement.scrollTop) /
      100 /
      2;
    ...
    <Container height={getWindowHeight(2)}>
      ...
      {isTouchDevice() ? (
        <div style={{ position: 'relative' }}>
          <TouchScrollable>
            <ScrollArea
              height={scrollAreaRef.current?.clientHeight ?? 0}
              innerRef={scrollAreaRef}
            >
              <p>
